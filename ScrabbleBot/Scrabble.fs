...
module State = 
    ...
    let mkState b d pn p h = {board = b; dict = d;  playerNumber = pn; points = p; hand = h }
...
module Scrabble =
    ...
    let playGame cstream pieces (st : State.state) =
        ...
        let rec aux (st : State.state) =
            ...
            match msg with
            | RCM (CMPlaySuccess(ms, points, newPieces)) -> //Only for us who made the move
                (* Successful play by you. Update your state (remove old tiles, add the new ones, change turn, etc) *)
                
                let newBoard = st.board
                //let newBoard = Parser.mkBoard 
                ...
                let st' = State.mkState newBoard st.dict st.playerNumber (st.points + points) handAddNew
    ...
    let startGame 
        ...
        fun () -> playGame cstream tiles (State.mkState board dict playerNumber 0 handSet)
